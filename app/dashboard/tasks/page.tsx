      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, width: 520, maxWidth: '90vw' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input style={inp} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Aufgabentitel..." />
          </div>
          <div>
            <label style={lbl}>Beschreibung</label>
            <textarea style={{ ...inp, height: 70, resize: 'vertical' }} value={form.description || ''} onChange={e => setForm({ ...form, description: e.target.value })} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              ...
              <input type="number" style={inp} value={form.points_value} onChange={e => setForm({ ...form, points_value: Number(e.target.value) })} />
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, marginTop: 20, justifyContent: 'flex-end' }}>
          ...
          <button onClick={onSave} disabled={!form.title || saving} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: form.title ? 'var(--accent)' : 'var(--border)', color: '#fff', cursor: form.title ? 'pointer' : 'not-allowed', fontWeight: 600 }}>
...
export default function TasksPage() {
  ...
  useEffect(() => {
    load()
    createClient().auth.getUser().then(({ data }: { data: any }) => setUserId(data.user?.id ?? null))
  }, [load])
      ...
      {loading ? <div style={{ color: 'var(--muted)', textAlign: 'center', padding: 40 }}>Lade...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {filtered.map((task: any) => {
            ...
            return (
              <div key={task.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderLeft: `3px solid ${cc}`, borderRadius: 12, padding: 18, display: 'flex', alignItems: 'center', gap: 16, cursor: 'pointer', transition: 'transform 0.15s' }}
                onMouseEnter={e => (e.currentTarget.style.transform = 'translateX(3px)')}
                onMouseLeave={e => (e.currentTarget.style.transform = 'none')}
              >
                <div style={{ flex: 1 }} onClick={() => openEdit(task)}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {assignee && <Tag label={assignee.full_name} color="var(--accent)" />}
                    {!assignee && <Tag label="Nicht zugewiesen" color="var(--muted)" />}
                ...
                <div style={{ textAlign: 'center', minWidth: 56 }} onClick={() => openEdit(task)}>
