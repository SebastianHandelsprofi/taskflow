...
export default function TeamsPage() {
  ...
  async function loadData() {
    const [tRes, pRes, tenantRes] = await Promise.all([fetch('/api/teams'), fetch('/api/profiles'), fetch('/api/tenant')])
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch('/api/teams', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newName, color: newColor }) })
  ...
  async function handleEdit() {
    ...
    setSaving(true)
    try {
      const res = await fetch('/api/teams', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editingTeam.id, name: editName, color: editColor, oldName: editingTeam.name }) })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
  ...
  async function handleAssign() {
    ...
    setAssigning(true)
    await fetch('/api/teams/members', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: selectedUser, abteilung: assignModal.name, role: selectedRole }) })
  ...
  async function handleRemove(userId: string) {
    ...
    await fetch('/api/teams/members', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId }) })
    loadData()
  }

  async function handleSetLeader(userId: string, abteilung: string) {
    await fetch('/api/teams/members', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, abteilung, role: 'bereichsleiter' }) })
    loadData()
  }
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '300px 1fr', gap: 24 }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, alignSelf: 'start' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
              {COLORS.map(c => (<div key={c} onClick={() => setNewColor(c)} style={{ width: 26, height: 26, borderRadius: 13, background: c, cursor: 'pointer', border: newColor === c ? '3px solid white' : '3px solid transparent', boxShadow: newColor === c ? `0 0 0 2px ${c}` : 'none', transition: 'all 0.2s' }} />))}
            </div>
          </div>
        ...
        <div>
          ...
          {!loading && teams.length === 0 && <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 40, textAlign: 'center', color: 'var(--muted)' }}>Noch keine Abteilungen</div>}
                  ...
                  <div>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                        {COLORS.map(c => (<div key={c} onClick={() => setEditColor(c)} style={{ width: 24, height: 24, borderRadius: 12, background: c, cursor: 'pointer', border: editColor === c ? '3px solid white' : '3px solid transparent', boxShadow: editColor === c ? `0 0 0 2px ${c}` : 'none' }} />))}
                      </div>
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--yellow)', marginBottom: 12, padding: '8px 10px', borderRadius: 6, background: '#ffd16618', border: '1px solid #ffd16633' }}>⚠️ Bei Namensänderung werden alle Mitglieder automatisch aktualisiert.</div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button onClick={handleEdit} disabled={saving} style={{ flex: 1, padding: '8px', borderRadius: 6, border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 12 }}>{saving ? 'Speichere...' : '✓ Speichern'}</button>
                    </div>
                  </div>
                ) : (
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: team.members?.length > 0 ? 12 : 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 34, height: 34, borderRadius: 8, background: `${team.color}22`, border: `2px solid ${team.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, color: team.color }}>{team.name[0]}</div>
                        <div>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button onClick={() => handleDelete(team.id)} disabled={deleting === team.id} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid #ff4d6d44', background: '#ff4d6d18', color: 'var(--red)', cursor: 'pointer', fontSize: 11 }}>{deleting === team.id ? '...' : '🗑'}</button>
                      </div>
                    </div>
                        ...
                        {team.members.map((m: any) => (
                          <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 8px', borderRadius: 8, background: 'var(--surface)', border: '1px solid var(--border)' }}>
                            ...
                            <div style={{ display: 'flex', gap: 3 }}>
                              {gamificationEnabled && m.role !== 'bereichsleiter' && (
                              ...
                              {!gamificationEnabled && m.role !== 'bereichsleiter' && (
                                <button onClick={() => handleSetLeader(m.id, team.name)} title="Als Bereichsleiter setzen" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 10, opacity: 0.5, color: 'var(--muted)' }}>BL</button>
                              )}
                    ...
                    {(!team.members || team.members.length === 0) && <div style={{ fontSize: 12, color: 'var(--muted)' }}>Noch keine Mitglieder</div>}
                  </div>
                )}
              </div>
            ))}
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: isMobile ? '20px 20px 0 0' : 16, padding: 28, width: isMobile ? '100%' : 420 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select style={inp} value={selectedUser} onChange={e => setSelectedUser(e.target.value)}>
                ...
                {profiles.map((p: any) => (<option key={p.id} value={p.id}>{p.full_name} {p.abteilung ? `(${p.abteilung})` : '(ohne Abteilung)'}</option>))}
              </select>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleAssign} disabled={!selectedUser || assigning} style={{ flex: isMobile ? 1 : 'none', padding: '10px 18px', borderRadius: 8, border: 'none', background: selectedUser ? 'var(--accent)' : 'var(--border)', color: '#fff', cursor: selectedUser ? 'pointer' : 'not-allowed', fontWeight: 600 }}>{assigning ? 'Zuordnen...' : 'Zuordnen'}</button>
            </div>
          </div>
        </div>
      )}
