...
export default function CategoriesPage() {
  ...
  async function handleEdit(id: string) {
    ...
    try {
      const res = await fetch('/api/categories', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, name: editName, color: editColor, icon: editIcon })
      })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      setEditingId(null)
  ...
  async function handleDelete(id: string, name: string) {
    ...
    if (taskCount > 0) {
      if (!confirm(`Diese Kategorie hat ${taskCount} Aufgaben. Trotzdem löschen?`)) return
    } else {
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24, alignSelf: 'start' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {ICONS.map(icon => (
                <button key={icon} onClick={() => setNewIcon(icon)} style={{ width: 34, height: 34, borderRadius: 8, border: `2px solid ${newIcon === icon ? 'var(--accent)' : 'var(--border)'}`, background: newIcon === icon ? 'var(--accent)22' : 'var(--surface)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {icon}
                </button>
              ))}
            </div>
                    ...
                    <div>
                      ...
                      <div style={{ marginBottom: 10 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                          {ICONS.map(icon => (
                            <button key={icon} onClick={() => setEditIcon(icon)} style={{ width: 30, height: 30, borderRadius: 6, border: `2px solid ${editIcon === icon ? 'var(--accent)' : 'var(--border)'}`, background: editIcon === icon ? 'var(--accent)22' : 'var(--surface)', cursor: 'pointer', fontSize: 14 }}>
                              {icon}
                            </button>
                          ))}
                        </div>
                      ...
                      <div style={{ marginBottom: 14 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          {COLORS.map(c => (
                            <div key={c} onClick={() => setEditColor(c)} style={{ width: 22, height: 22, borderRadius: 11, background: c, cursor: 'pointer', border: editColor === c ? '3px solid white' : '3px solid transparent', boxShadow: editColor === c ? `0 0 0 2px ${c}` : 'none' }} />
                          ))}
                        </div>
                      </div>
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 40, height: 40, borderRadius: 10, background: `${cat.color}22`, border: `2px solid ${cat.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                            {cat.icon}
                          </div>
                          <div>
                            <div style={{ fontSize: 15, fontWeight: 700, color: cat.color }}>{cat.name}</div>
                            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                              {taskCount > 0 ? `${taskCount} Aufgabe${taskCount !== 1 ? 'n' : ''}` : 'Keine Aufgaben'}
                            </div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: 6 }}>
                          ...
                          <button onClick={() => handleDelete(cat.id, cat.name)} disabled={deleting === cat.id} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid #ff4d6d44', background: '#ff4d6d18', color: 'var(--red)', cursor: 'pointer', fontSize: 11 }}>
                            {deleting === cat.id ? '...' : '🗑'}
                          </button>
                        </div>
                      </div>
                        ...
                        <div>
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {(['Offen', 'In Bearbeitung', 'Erledigt', 'Ueberfaellig'] as const).map(status => {
                              ...
                              return (
                                <div key={status} style={{ padding: '3px 8px', borderRadius: 4, background: `${colors[status]}22`, border: `1px solid ${colors[status]}44`, fontSize: 11, color: colors[status], fontWeight: 600 }}>
                                  {count} {labels[status]}
                                </div>
                              )
                            })}
